	...
	switch (type)
	{
		...
		case EventType::MouseMove:
		{
			...
			if (isVisible && GetParent()->GetUIItemDragBehavior() == "MOVE")
			{
				...
				{
					itemCopy = new UIItem(uiComponents, worldPos, uiLayer, 2, eventHandler, itemId, d2dDeviceContext, d2dFactory, d3dDevice, d3dDeviceContext, vertexShader, pixelShader, texture, grayTexture, highlightBrush, vertexShaderBuffer, vertexShaderSize, clientWidth, clientHeight, projectionTransform, true, mousePosX, mousePosY);
		...
		case EventType::RightMouseDown:
		{
			...
			if (isVisible && GetParent()->GetUIItemRightClickBehavior() == "LOOT")
			{
				// move item from LootContainer into player's Inventory
			}
